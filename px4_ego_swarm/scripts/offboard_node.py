...
class Offboard:
    ...
    def __init__(self):
        ...
        ros services
        '''
        self.armServer = rospy.ServiceProxy('/drone0/mavros/cmd/arming', CommandBool)
        self.setModeServer = rospy.ServiceProxy('/drone0/mavros/set_mode', SetMode)
        
        ''' 
        ros publishers
        '''
        self.local_target_pub = rospy.Publisher('/drone0/mavros/setpoint_raw/local', PositionTarget, queue_size=10)
        
        '''
        ros subscribers
        '''
        self.mavros_state_sub = rospy.Subscriber("/drone0/mavros/state", State, self.mavros_state_callback)
        self.local_pose_sub = rospy.Subscriber("/drone0/mavros/local_position/pose", PoseStamped, self.local_pose_callback, queue_size=1)
